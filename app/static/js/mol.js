...
mol.modules.map.layers = function(mol) {
  ...
  mol.map.layers.LayerEngine = mol.mvp.Engine.extend({
    ...
    layersToggle: function(event) {
      ...
      if (this.display.expanded == true || visible == false) {
        ...
        this.display.layersWrapper.animate({
          ...
          self.display.layersToggle.text('â–¼');
        ...
        this.display.layersWrapper.animate({
          height: this.display.layersHeader.height() + this.display.layersContainer.height() + 35
        }, 1000, function() {
          self.display.layersToggle.text('â–²');
    ...
    addEventHandlers: function() {
      ...
      function(event) {
        ...
        _.each(
        $(self.display).find(".toggle"), function(checkbox) {
          checkbox.click({
            currentTarget: this
          })
        });
      ...
      function(event) {
        ...
        _.each(
        self.display.layers, function(layer) {
          var l, o;
          ...
          $(l).find('.styler'), o, layer, layer.orig_opacity);
      ...
      function(event) {
        ...
        baseHtml = '' + '<div class="mol-LayerControl-Styler">' + '  <div class="colorPickers">' + '    <div class="colorPicker">' + '      <span class="stylerLabel">Color:&nbsp</span>' + '      <input type="text" id="allFill" />' + '    </div>' + '  </div>' + '  <div class="buttonWrapper allStyler">' + '    <button id="applyStyle">Apply</button>' + '    <button id="cancelStyle">Cancel</button>' + '  </div>' + '</div>';
        ...
        q = $(button).qtip({
          ...
          events: {
            render: function(event, api) {
              var colors = ['black', 'white', 'red', 'yellow', 'blue', 'green', 'orange', 'purple'],
                colors2 = ['#66C2A5', '#FC8D62', '#8DA0CB', '#E78AC3', '#A6D854', '#FFD92F', '#E5C494'];
              ...
              function(event) {
                ...
                color = $('#allFill').spectrum("get").toHexString();
              ...
              $(api.elements.content).find('#cancelStyle').click(
      ...
      this.bus.addHandler('layer-opacity', function(event) {
      ...
      this.bus.addHandler('layer-display-toggle', function(event) {
        var params = null,
          e = null;
        ...
        if (event.visible === undefined) {
          self.display.toggle();
          params = {
            visible: self.display.is(':visible')
          };
        } else {
      ...
      this.bus.addHandler('layers-toggle', function(event) {
        self.layersToggle(event);
      });

      this.bus.addHandler('layer-click-toggle', function(event) {
        ...
        if (event.disable) {
          ...
          _.any($(self.display.list).children(), function(layer) {
            if ($(layer).find('.layer').hasClass('selected')) {
              self.map.overlayMapTypes.forEach(
    ...
    addLayers: function(layers) {
      ...
      sortedLayers, function(layer) {
        ...
        function(event) {
          ...
          if (self.map.overlayMapTypes.length == 0) {
            self.bus.fireEvent(
            new mol.bus.Event('hide-layer-display-toggle'));
        ...
        function(event) {
          ...
          if ($(this).hasClass('selected')) {
            ...
            if ($(self.display).find('.selected').length > 0) {
              ...
              self.display.getLayerById(
              $(self.display).find('.selected').parent().attr('id')), false);
            }
        ...
        function(event) {
          self.bus.fireEvent(
          new mol.bus.Event('metadata-toggle', {
            params: {
      ...
      this.bus.fireEvent(
      new mol.bus.Event('reorder-layers', {
        layers: layerIds
      }));
      ...
      if (sortedLayers.length == 1) {
        ...
        //select it
        this.display.list.find('.layer')[this.display.list.find('.layer').length - 1].click();
    ...
    displayLayerStyler: function(button, layer) {
      var baseHtml, layer_curr_style, layer_orig_style, max, min, params = {
        layer: layer,
        style: null
      },
        q, self = this;
      ...
      baseHtml = '' + '<div class="mol-LayerControl-Styler ' + layer.source + '">' + '  <div class="colorPickers"></div>' + '  <div class="sizerHolder"></div>' + '  <div class="opacityHolder">' + '    <span class="sliderLabel">Opacity:&nbsp</span>' + '    <div class="sliderContainer">' + '      <div class="opacity"></div>' + '    </div>' + '    <span id="opacityValue">50</span>' + '  </div>' + '  <div class="buttonWrapper">' + '    <button id="applyStyle">Apply</button>' + '    <button id="resetStyle">Reset</button>' + '    <button id="cancelStyle">Cancel</button>' + '  </div>' + '</div>';
      ...
      q = $(button).qtip({
        ...
        events: {
          render: function(event, api) {
            ...
            $(api.elements.content).find('.mol-LayerControl-Styler'), layer);

            self.setStylerProperties(
            api.elements.content, layer, layer_curr_style, layer_orig_style, false);
            ...
            function(event) {
              ...
              if (layer.type == "range") {
                o.s1 = $('#showFill1Palette').spectrum("get").toHexString();
                o.s2 = $('#showFill2Palette').spectrum("get").toHexString();
                o.s3 = $('#showFill3Palette').spectrum("get").toHexString();
                o.s4 = $('#showFill4Palette').spectrum("get").toHexString();

                if (layer.source == "iucn") {
                  o.s5 = $('#showFill5Palette').spectrum("get").toHexString();
                }
              } else {
                o.fill = $('#showFillPalette').spectrum("get").toHexString();
              }

              o.border = $('#showBorderPalette').spectrum("get").toHexString();
              o.size = $(api.elements.content).find('.sizer').slider('value');

              self.updateLegendCss(
              button, o, layer, parseFloat($(api.elements.content).find('.opacity').slider("value")));
            ...
            $(api.elements.content).find('#resetStyle').click(

            function(event) {
              self.setStylerProperties(
              api.elements.content, layer, layer_orig_style, layer_orig_style, true);
            });
    ...
    displayHabitatClipping: function(button, layer) {
      ...
      baseHtml = '' + '<div class="mol-LayerControl-Styler">' + '<div class="habitats"></div>' + '<br>' + '<div>Habitat from MODIS MCD12Q1 V005</div>' + '<div>Choose Year' + '<select class="year">' + '<option value="2001">2001</option>' + '<option value="2002">2002</option>' + '<option value="2003">2003</option>' + '<option value="2004">2004</option>' + '<option value="2005">2005</option>' + '<option value="2006">2006</option>' + '<option value="2007">2007</option>' + '</select>' + '</div>' + '<div class="elevLabel">' + 'Elevation Range:<br>0m-10000m' + '</div>' + '<div class="elev"></div>' + '<div class="buttonWrapper">' + '<button class="apply">Apply</button>' + '<button class="reset">Reset</button>' + '<button class="cancel">Cancel</button>' + '</div>' + '</div>';
      ...
      q = $(button).qtip({
        ...
        events: {
          render: function(event, api) {
            ...
            api.elements.content, layer, false);
            ...
            $(api.elements.content).find('.reset').click(

            function(event) {
              self.setHabitatProperties(
              api.elements.content, layer, true);
            });

            $(api.elements.content).find('.cancel').click(
    ...
    setHabitatProperties: function(cont, layer, reset) {
      ...
      _.each(
      habitats, function(habitat, habitat_code) {
        var html = '' + '<div class="habitat {0}" ' + 'data-habitat="{1}">{2}</div>',
          display = $(html.format(
          (_.indexOf(layer.selectedHabitats, habitat_code) >= 0) ? 'selected' : '', habitat_code, habitat));
        display.click(

        function(event) {
          if ($(this).hasClass('selected')) {
            $(this).removeClass('selected');
            layer.selectedHabitats = _.without(
            layer.selectedHabitats, [$(this).data('habitat').toString()]);
          } else {
            $(this).addClass('selected');
      ...
      $(cont).find('.elev').slider({
        ...
        slide: function(event, ui) {
          $(cont).find('.elevLabel').html('Elevation Range:<br>{0}m-{1}m'.format(ui.values[0], ui.values[1]));
          layer.selectedElev = ui.values;
        }.bind(layer)
      });

      $(cont).find('.elevLabel').html('Elevation Range:<br>{0}m-{1}m'.format(layer.selectedElev[0], layer.selectedElev[1]));
    ...
    getStylerLayout: function(element, layer) {
      ...
      if (layer.style_table == "points_style") {
        pickers = '' + '<div class="colorPicker">' + '  <span class="stylerLabel">Fill:&nbsp</span>' + '  <input type="text" id="showFillPalette" />' + '</div>' + '<div class="colorPicker">' + '  <span class="stylerLabel">Border:&nbsp</span>' + '  <input type="text" id="showBorderPalette" />' + '</div>';

        sizer = '' + '<span class="sliderLabel">Size:&nbsp</span>' + '  <div class="sliderContainer">' + '    <div class="sizer"></div>' + '  </div>' + '<span id="pointSizeValue">8px</span>';

        $(element).find('.colorPickers').prepend(pickers);
        $(element).find('.sizerHolder').prepend(sizer);
      } else {
        if (layer.type == "range") {
          pickers = '' + '<span class="seasonLabel">Breeding</span>' + '<div class="colorPicker">' + '  <span class="stylerLabel">Fill:&nbsp</span>' + '  <input type="text" id="showFill2Palette" />' + '</div>' + '<span class="seasonLabel">Resident</span>' + '<div class="colorPicker">' + '  <span class="stylerLabel">Fill:&nbsp</span>' + '  <input type="text" id="showFill1Palette" />' + '</div>' + '<span class="seasonLabel">Non-breeding</span>' + '<div class="colorPicker">' + '  <span class="stylerLabel">Fill:&nbsp</span>' + '  <input type="text" id="showFill3Palette" />' + '</div>' + '<span class="seasonLabel">Passage</span>' + '<div class="colorPicker">' + '  <span class="stylerLabel">Fill:&nbsp</span>' + '  <input type="text" id="showFill4Palette" />' + '</div>';

          if (layer.source == "iucn") {
            pickers += '' + '<span class="seasonLabel">' + 'Seasonality Uncertain</span>' + '<div class="colorPicker">' + '  <span class="stylerLabel">Fill:&nbsp</span>' + '  <input type="text" id="showFill5Palette" />' + '</div>';
          }

          pickers += '' + '<span class="seasonLabel">All</span>' + '<div class="colorPicker">' + '  <span class="stylerLabel">Border:&nbsp</span>' + '  <input type="text" id="showBorderPalette" />' + '</div>';

          sizer = '' + '<span class="sliderLabel">Width:&nbsp</span>' + '  <div class="sliderContainer">' + '    <div class="sizer"></div>' + '  </div>' + '<span id="pointSizeValue">8px</span>';
    ...
    parseLayerStyle: function(layer, original) {
      var o, fillStyle, borderStyle, sizeStyle, style, s1Style, s2Style, s3Style, s4Style, s5Style, s1, s2, s3, s4, s5;
      ...
      if (layer.style_table == "points_style") {
        ...
        style.indexOf('marker-fill'), style.length - 1);

        borderStyle = style.substring(
        style.indexOf('marker-line-color'), style.length - 1);

        sizeStyle = style.substring(
        style.indexOf('marker-width'), style.length - 1);

        o = {
          fill: fillStyle.substring(
          fillStyle.indexOf('#'), fillStyle.indexOf(';')),
          border: borderStyle.substring(
          borderStyle.indexOf('#'), borderStyle.indexOf(';')),
          size: Number($.trim(sizeStyle.substring(
          sizeStyle.indexOf(':') + 1, sizeStyle.indexOf(';'))))
        };
      } else {
        if (layer.type == "range") {
          s1Style = style.substring(
          style.indexOf('seasonality=1'), style.length - 1);

          s1 = s1Style.substring(
          s1Style.indexOf('polygon-fill'), s1Style.length - 1);

          s2Style = style.substring(
          style.indexOf('seasonality=2'), style.length - 1);

          s2 = s2Style.substring(
          s2Style.indexOf('polygon-fill'), s2Style.length - 1);

          s3Style = style.substring(
          style.indexOf('seasonality=3'), style.length - 1);

          s3 = s3Style.substring(
          s3Style.indexOf('polygon-fill'), s3Style.length - 1);

          s4Style = style.substring(
          style.indexOf('seasonality=4'), style.length - 1);

          s4 = s4Style.substring(
          s4Style.indexOf('polygon-fill'), s4Style.length - 1);

          o = {
            s1: s1.substring(
            s1.indexOf('#'), s1.indexOf(';')),
            s2: s2.substring(
            s2.indexOf('#'), s2.indexOf(';')),
            s3: s3.substring(
            s3.indexOf('#'), s3.indexOf(';')),
            s4: s4.substring(
          ...
          if (layer.source == "iucn") {
            ...
            style.indexOf('seasonality=5'), style.length - 1);

            s5 = s5Style.substring(
            s5Style.indexOf('polygon-fill'), s5Style.length - 1);

            o.s5 = s5.substring(
            s5.indexOf('#'), s5.indexOf(';'));
          }
        } else {
          fillStyle = style.substring(
          style.indexOf('polygon-fill'), style.length - 1);
        ...
        borderStyle = style.substring(
        style.indexOf('line-color'), style.length - 1);

        sizeStyle = style.substring(
        style.indexOf('line-width'), style.length - 1);
    ...
    changeStyleProperty: function(style, prop, newSty, isSeas, seasonProp) {
      var updatedStyle, subStyle, spreStyle, preStyle, smidStyle, midStyle, srestStyle;

      if (isSeas) {
        spreStyle = style.substring(
        0, style.indexOf("seasonality=" + prop + "]"));

        preStyle = style.substring(
        style.indexOf("seasonality=" + prop + "]"), style.length);

        smidStyle = preStyle.substring(
        0, preStyle.indexOf(seasonProp + ":"));

        midStyle = preStyle.substring(
        preStyle.indexOf(seasonProp + ":"), preStyle.length);

        srestStyle = midStyle.substring(
        midStyle.indexOf(";"), midStyle.length);

        updatedStyle = spreStyle + smidStyle + seasonProp + ":" + newSty + srestStyle;
      } else {
        subStyle = style.substring(style.indexOf(prop), style.length);

        updatedStyle = style.substring(
        0, style.indexOf(prop + ":") + prop.length + 1) + newSty + subStyle.substring(
        subStyle.indexOf(";"), subStyle.length);
      }
    ...
    updateStyle: function(layer, style, newStyle) {
      var updatedStyle, season;

      if (layer.style_table == "points_style") {
        style = this.changeStyleProperty(
        style, 'marker-fill', newStyle.fill, false);
        style = this.changeStyleProperty(
        style, 'marker-line-color', newStyle.border, false);
        style = this.changeStyleProperty(
        ...
        if (layer.type == "range") {
          ...
          style = this.changeStyleProperty(
          style, 'polygon-fill', newStyle.fill, false);
        }
    ...
    updateLegendCss: function(button, o, layer, opa) {
      if (layer.type == "range") {
        ...
        $(button).find('.legend-seasonal').css({
        ...
        if (layer.style_table == "points_style") {
          ...
          $(button).find('.legend-polygon').css({
            'background-color': o.fill,
            'border-color': o.border,
            'border-width': o.size + "px",
            'opacity': opa
    ...
    updateLayerStyle: function(button, obj, lay, opa) {
      var o = obj,
        os = {},
        sel_style_desc, style_desc, params = {},
        oparams = {},
        self = this;
      ...
      self.bus.fireEvent(new mol.bus.Event('layer-opacity', oparams));
    },

    toggleLayerHighlight: function(layer, visible) {
      var o = {},
        style_desc, self = this,
        style = layer.tile_style,
        oldStyle, params = {
          layer: layer,
          style: null
      ...
      if (layer.style_table == "points_style") {
        style = this.changeStyleProperty(
        style, 'marker-line-color', visible ? '#FF00FF' : oldStyle.border, false);
      } else {
        style = this.changeStyleProperty(
        style, 'line-color', visible ? '#FF00FF' : oldStyle.border, false);

        style = this.changeStyleProperty(
        style, 'line-width', visible ? 1 : oldStyle.size, false);
      }
      ...
      self.bus.fireEvent(
      new mol.bus.Event('apply-layer-style', params));
    },
    ...
    initSortable: function() {
      ...
      display.list.sortable({
        update: function(event, ui) {
          ...
          $(display.list).find('.layerContainer').each(function(i, el) {
  ...
  mol.map.layers.LayerDisplay = mol.mvp.View.extend({
    init: function(layer) {
      var html = '' + '<div class="layerContainer">' + '  <div class="layer">' + '    <button title="Click to edit layer style." ' + 'class="styler">' + '      <div class="legend-point"></div> ' + '      <div class="legend-polygon"></div> ' + '      <div class="legend-seasonal">' + '        <div class="seasonal s1"></div>' + '        <div class="seasonal s2"></div>' + '        <div class="seasonal s3"></div>' + '        <div class="seasonal s4"></div>' + '        <div class="seasonal s5"></div>' + '      </div> ' + '    </button>' + '    <button class="source" title="Layer Source: {5}">' + '      <img src="/static/maps/search/{0}.png">' + '    </button>' + '    <button class="type" title="Layer Type: {6}">' + '      <img src="/static/maps/search/{1}.png">' + '    </button>' + '    <div class="layerName">' + '      <div class="layerRecords">{4}</div>' + '      <div title="{2}" class="layerNomial">{2}</div>' + '      <div title="{3}" class="layerEnglishName">{3}</div>' + '    </div>' + '    <button title="Remove layer." class="close buttons">' + 'x' + '    </button>' + '    <button title="Zoom to layer extent." class="zoom buttons">' + 'z' + '    </button>' + '    <label class="buttonContainer">' + '       <input class="toggle" type="checkbox">' + '       <span title="Toggle layer visibility." ' + 'class="customCheck"></span>' + '    </label>' + '    <button title="Apply habitat filters." class="habitat buttons">' + '<img src="/static/maps/layers/habitat.png">' + '    </button>' + '   </div>' + '   <div class="break"></div>' + '</div>',
        self = this;

      this._super(
      html.format(
      layer.source_type, layer.type, layer.name, layer.names, (layer.feature_count != null) ? '{0} features'.format(layer.feature_count) : '', layer.source_title, layer.type_title));
  ...
  mol.map.layers.LayerListDisplay = mol.mvp.View.extend({
    init: function() {
      var html = '' + '<div class="mol-LayerControl-Layers">' + '<div class="layers widgetTheme">' + '<div class="layersHeader">' + '<button class="layersToggle button">â–²</button>' + 'Layers' + '</div>' + '<div class="layersContainer">' + '<div class="scrollContainer">' + '<div id="sortable"></div>' + '</div>' + '<div class="pageNavigation">' + '<button class="removeAll">' + 'Remove All' + '</button>' + '<button class="toggleAll">' + 'Toggle All' + '</button>' + '<button class="resetAll">' + 'Reset All' + '</button>' + '<button class="styleAll">' + 'Style All' + '</button>' + '</div>' + '</div>' + '</div>' + '</div>';
    ...
    sort_by: function(layers_order) {
      this.layers.sort(function(a, b) {
        return _(layers_order).indexOf(a.name) - _(layers_order).indexOf(b.name);
      });
      this.open = true;
      this.hiding();
    },
