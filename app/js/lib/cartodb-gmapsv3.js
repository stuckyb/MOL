    ...
    google.maps.CartoDBLayer = function (params) {
        ...
        if (this.params.infowindow) {
	     ...
	     function addCartoDBTiles(params) {
	         ...
	         var cartodb_layer = {
	             getTileUrl: function(coord, zoom) {
	                 return 'http://' + this.params.user_name + '.' + this.params.tile_host + '/tiles/' + this.params.table_name + '/'+zoom+'/'+coord.x+'/'+coord.y+'.png?sql='+ (this.params.query|| '') + '&map_key=' + (this.params.map_key || '');
	             },
	     ...
	     function addSimpleCartoDBTiles(params) {
	         ...
	         var cartodb_layer = {
	             getTileUrl: function(coord, zoom) {
	                 return 'http://' + params.user_name + '.' + this.params.tile_host + '/tiles/' + params.table_name + '/'+zoom+'/'+coord.x+'/'+coord.y+'.png?sql='+params.query + '&map_key=' + (params.map_key || '');
	             },
	             tileSize: new google.maps.Size(256, 256),
                name: this.params.id, //params.query,
                description: false
	         };
        ...
        function refreshTiles(params,sql) {
            ...
            if (!params.infowindow) {
                ...
                removeOldLayer(params.map,params.layer);
    		       ...
    		       var cartodb_layer = {
    			        getTileUrl: function(coord, zoom) {
    			            return 'http://' + params.user_name + '.' + this.params.host + '/tiles/' + params.table_name + '/'+zoom+'/'+coord.x+'/'+coord.y+'.png?sql='+params.query + '&map_key=' + (params.map_key || '');
    		           },
  			           tileSize: new google.maps.Size(256, 256),
                    name: params.id, //params.query,
                    description: false
  	             };
