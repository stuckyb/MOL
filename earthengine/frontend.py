...
class MergeFeatureCollection(EarthEngineRequest):
    ...
    def get_url(self):
        ...
        if count == 3:
            table['collection'] = self.get_collection(self.tableids.pop())        
            table['collection']['collection2'] = self.get_collection(
                self.tableids.pop())
            table['collection']['collection2']['collection2'] = dict(
                table_id=self.tableids.pop())

        elif count == 2:
            table['collection'] = dict(
                algorithm = "MOL/com.google.earthengine.examples.mol.MergeFeatureCollection",
                collection1 = dict(table_id = self.tableids.pop()),
                collection2 = dict(table_id = self.tableids.pop()))
